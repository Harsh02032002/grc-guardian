...
export default function ConfigLocation() {
  ...
  const handleAdd = () => {
    ...
    setItems([...items, { id: items.length + 1, name: newName, address: newAddress }]);
  ...
  return (
    <div className="page-container">
      ...
      <table className="data-table">
        <thead><tr><th>#</th><th>Location Name</th><th>Address / Details</th><th>Actions</th></tr></thead>
        <tbody>
          {items.map((item, idx) => (
            <tr key={item.id}>
              <td className="font-mono text-xs">{idx + 1}</td>
              <td className="font-medium"><span className="inline-flex items-center gap-1"><MapPin className="h-3 w-3 text-primary" />{item.name}</span></td>
