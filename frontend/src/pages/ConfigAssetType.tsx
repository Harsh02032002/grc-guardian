...
import { masterAssetTypes } from "@/data/masterData";
import { Input } from "@/components/ui/input";
import { Label } from "@/components/ui/label";
import { Edit, Trash2, Plus } from "lucide-react";

export default function ConfigAssetType() {
  const [items, setItems] = useState(masterAssetTypes);
  ...
  const handleAdd = () => {
    ...
    setItems([...items, { id: items.length + 1, name: newName, description: newDesc }]);
    ...
    <div className="page-container">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="page-subtitle">Manage asset type classifications (Tangible, Scenario, Process, etc.)</p>
        </div>
      ...
      {showAdd && (
        <div className="bg-card rounded-lg border shadow-sm p-4 space-y-3">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div className="space-y-1.5"><Label>Type Name *</Label><Input value={newName} onChange={e => setNewName(e.target.value)} placeholder="e.g., Digital Asset" /></div>
      ...
      <table className="data-table">
        <thead><tr><th>#</th><th>Asset Type</th><th>Description</th><th>Actions</th></tr></thead>
        <tbody>
          {items.map((item, idx) => (
            <tr key={item.id}>
