...
export default function AddRisk() {
  ...
  const [riskSubcategories, setRiskSubcategories] = useState<any[]>([]);
  const [riskOwners, setRiskOwners] = useState<any[]>([]);
  ...
  const pcr = cr1;
  const ccp = cr2;
  const revisedRIR = probability * impact;
  ...
  return (
    <div className="page-container max-w-7xl">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-4 gap-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          <div className="space-y-1.5">
            <Label className="text-xs">Probability of Occurrence </Label>
        ...
        <div className="grid grid-cols-2 gap-3 bg-muted/40 rounded-md p-4 border">
          <CalcBox label="" value={absoluteRIR} color={rirColor(absoluteRIR)} />
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          <div className="space-y-1.5">
            <Label className="text-xs">Impact of Occurrence </Label>
        ...
        <div className="grid grid-cols-2 gap-3 bg-muted/40 rounded-md p-4 border">
          <CalcBox label="P × I " value={revisedRIR} color={rirColor(revisedRIR)} />
        ...
        <div className="lg:col-span-1">
          <div className="sticky top-4 space-y-4">
            ...
            <div className="bg-card rounded-lg border shadow-sm p-4">
              ...
              <div className="space-y-3 text-xs">
                {riskName && (
                  <div>
                    ...
                    <p className="truncate mt-1">{riskName}</p>
                  </div>
                )}
                ...
                {subcategory && (
                  <div>
                    ...
                    <p className="mt-1">{subcategory}</p>
                  </div>
                )}
                
                {selectedRiskOwner && (
                  <div>
                    <span className="font-medium text-muted-foreground">Risk Owner:</span>
                    <p className="mt-1">{selectedRiskOwner}</p>
                  </div>
                )}
                ...
                {selectedAsset && (
                  <div>
                    ...
                    <p className="truncate mt-1">{selectedAsset}</p>
                  </div>
                )}
              </div>

              {/* Risk Calculations */}
              {(absoluteRIR > 0 || revisedRIR > 0) && (
                <div className="border-t pt-3 mt-3">
                  <h4 className="text-xs font-semibold text-muted-foreground mb-2">Risk Calculations</h4>
                  <div className="grid grid-cols-2 gap-2">
                    <div className="text-center p-2 bg-muted/50 rounded">
                      <p className="text-[10px] text-muted-foreground">Absolute RIR</p>
                      <p className={`font-bold text-sm ${rirColor(absoluteRIR)}`}>{absoluteRIR}</p>
                      <p className="text-[10px]">{rirLabel}</p>
                    </div>
                    <div className="text-center p-2 bg-muted/50 rounded">
                      <p className="text-[10px] text-muted-foreground">Revised RIR</p>
                      <p className={`font-bold text-sm ${rirColor(revisedRIR)}`}>{revisedRIR}</p>
                      <p className="text-[10px]">{riskPriority}</p>
                    </div>
                  </div>
                </div>
              )}
              ...
              {/* Progress Indicator */}
              <div className="border-t pt-3 mt-3">
                <h4 className="text-xs font-semibold text-muted-foreground mb-2">Completion Progress</h4>
                <div className="space-y-1">
                  {[
                    { label: "Basic Info", done: !!(riskName && category && description) },
                    { label: "Asset", done: !!selectedAsset },
                    { label: "Risk Assessment", done: !!(probability > 0 && impact > 0) },
                    { label: "Controls", done: !!(pc_controls || cc_controls) },
                    { label: "Treatment", done: !!treatmentRequired }
                  ].map((item, idx) => (
                    <div key={idx} className="flex items-center gap-2 text-xs">
                      <span className={`w-3 h-3 rounded-full flex items-center justify-center ${item.done ? 'bg-green-500' : 'bg-muted'}`}>
            ...
            <div className="bg-card rounded-lg border shadow-sm p-4">
              ...
              <div className="space-y-2">
                <button 
                  ...
                  className="w-full text-xs px-3 py-2 bg-primary text-primary-foreground rounded hover:opacity-90 transition"
                >
                  + Add Category
                </button>
                <button 
                  onClick={() => {
                  if (category) setNewSubParent(category);
                  setShowSubCategoryModal(true);
                }}
                  className="w-full text-xs px-3 py-2 bg-secondary text-secondary-foreground rounded hover:opacity-90 transition"
                  disabled={!category}
                >
                  + Add Subcategory
                </button>
